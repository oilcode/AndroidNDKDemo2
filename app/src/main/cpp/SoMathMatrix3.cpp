...
//----------------------------------------------------------------
void SoMathMatrix3::MakeTranspose()
{
...
//----------------------------------------------------------------
void SoMathMatrix3::MakeInverse()
{
...
{
	float x = m[0][0] * kVector.x + m[1][0] * kVector.y + m[2][0] * kVector.z;
	float y = m[0][1] * kVector.x + m[1][1] * kVector.y + m[2][1] * kVector.z;
	float z = m[0][2] * kVector.x + m[1][2] * kVector.y + m[2][2] * kVector.z;
	return SoMathFloat3(x, y, z);
}
//----------------------------------------------------------------
SoMathFloat3 operator * (const SoMathFloat3& kVector, const SoMathMatrix3& kMatrix)
{
	float x = kMatrix.m[0][0] * kVector.x + kMatrix.m[1][0] * kVector.y + kMatrix.m[2][0] * kVector.z;
	float y = kMatrix.m[0][1] * kVector.x + kMatrix.m[1][1] * kVector.y + kMatrix.m[2][1] * kVector.z;
	float z = kMatrix.m[0][2] * kVector.x + kMatrix.m[1][2] * kVector.y + kMatrix.m[2][2] * kVector.z;
	return SoMathFloat3(x, y, z);
}
//----------------------------------------------------------------
...
#else
	SoMathMatrix3 kResult;
	kResult.m[0][0] = m[0][0]*kOther.m[0][0] + m[1][0]*kOther.m[0][1] + m[2][0]*kOther.m[0][2];
	kResult.m[0][1] = m[0][1]*kOther.m[0][0] + m[1][1]*kOther.m[0][1] + m[2][1]*kOther.m[0][2];
	kResult.m[0][2] = m[0][2]*kOther.m[0][0] + m[1][2]*kOther.m[0][1] + m[2][2]*kOther.m[0][2];
	//
	kResult.m[1][0] = m[0][0]*kOther.m[1][0] + m[1][0]*kOther.m[1][1] + m[2][0]*kOther.m[1][2];
	kResult.m[1][1] = m[0][1]*kOther.m[1][0] + m[1][1]*kOther.m[1][1] + m[2][1]*kOther.m[1][2];
	kResult.m[1][2] = m[0][2]*kOther.m[1][0] + m[1][2]*kOther.m[1][1] + m[2][2]*kOther.m[1][2];
	//
	kResult.m[2][0] = m[0][0]*kOther.m[2][0] + m[1][0]*kOther.m[2][1] + m[2][0]*kOther.m[2][2];
	kResult.m[2][1] = m[0][1]*kOther.m[2][0] + m[1][1]*kOther.m[2][1] + m[2][1]*kOther.m[2][2];
	kResult.m[2][2] = m[0][2]*kOther.m[2][0] + m[1][2]*kOther.m[2][1] + m[2][2]*kOther.m[2][2];
	//
	return kResult;
#endif
}
